...
#define ID_AREA 105
#define ID_RADIOBUTTON5 106
#define ID_RADIOBUTTON6 107
...
void DrawGraph(HDC hdc/*указатель на тип курсива*/, HWND hwnd) {
    bool flag = 0;
    ...
    for (x = -380; x <= 380; x += 1) {
        if (functionID == 1) {
            ...
            _asm {
                ...
                fld x
                fstp y
                fwait //ожидание завершения предыдущих операций
            }
        }
        ...
        while (a != 0 && yOld != 0)
        {
            //SetPixel(hdc, x + 200, 200 - (int)yOld + a, RGB(255, 204, 102));
            if (a > 0)
                a--;
            if (a < 0)
                a++;
        ...
        //график функции
        SetPixel(hdc, x + 200, 200 - (int)y, RGB(255, 100, 102));
...
LRESULT CALLBACK WndProc(HWND hwnd,//дескриптор окна
                        ...
                        WPARAM wParam,// параметры сообщений 
                        LPARAM lParam) //
{
    HDC hdc;
    PAINTSTRUCT ps;
    ...
    int x1, x2; // Новые переменные для хранения значений x1 и x2
    
    LPWSTR buffer = (LPWSTR)malloc(4 * sizeof(WCHAR)); // bufferSize - размер буфера в символа
    ...
    case WM_CREATE:
        ...
        hEdit1 = CreateWindow(
            L"EDIT", L"Helllo", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_LEFT,
       ...
       // Добавляем поле ввода для x2
       hEdit2= CreateWindow(TEXT("edit"), TEXT(""),
           WS_VISIBLE | WS_CHILD | ES_NUMBER,
           150, 250, 50, 20, hwnd, (HMENU)ID_RADIOBUTTON6, NULL, NULL);
    ...
    case WM_COMMAND:
        ...
        case ID_RADIOBUTTON5:
            ...
            functionID = 5;
            GetWindowText(hEdit1, buffer, 4*sizeof(WCHAR));
            x1 = _wtoi((wchar_t*)buffer);

            OutputDebugString(TEXT("Debug: ID_RADIOBUTTON5 pressed\n"));
            OutputDebugString(TEXT("Debug: x1 = "));
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    RegisterClass(&wc);
    hwnd = CreateWindow(wc.lpszClassName, TEXT("Graphs"),
